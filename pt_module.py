...
class StRefine(object):
    ...
    def refine(self, PoG_px, PoG_history, return_full=False,show=False):
        self.show=show
        ...
        # print(output_combmap_tensor.cpu().detach().numpy().shape)
        # 3. retreive refined gaze
        #st()
        n, _, h, w = output_combmap_tensor.shape
        gaze_heatmap = output_combmap_tensor[:, 0, :, :]
        history_refined_heatmap=output_combmap_tensor[:, 1, :, :]
        if (self.show):
            plt.imshow(history_refined_heatmap.cpu().detach().numpy()[0], origin='upper')
            plt.show()
